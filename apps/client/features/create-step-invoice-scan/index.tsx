...
// Wizard étape 3 - OCR mock pour pré-remplir la composition (V5 backend, cf. docs/PIVOT-PLAN.md).
...
export function CreateStepInvoiceScan({ draftId }: { draftId: string }) {
    ...
    return (
        <WizardStepFrame
            ...
            subtitle="Importez une facture fournisseur. L’OCR extrait fournisseur, fibres et quantités."
        ...
        >
            <label className="border-border bg-muted/40 hover:bg-muted relative flex h-44 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed transition-colors">
                ...
                <p className="text-foreground text-sm font-medium">{file ? file.name : 'Glissez une facture ici'}</p>
            ...
            {extracted && (
                <Card className="border-lumiris-emerald/30 bg-lumiris-emerald/5">
                    ...
                    <CardContent className="space-y-3 text-sm">
                        ...
                        />
                        <div>
                            <p className="text-muted-foreground mb-1.5 text-[11px] uppercase tracking-wider">Lignes</p>
                                ...
                                {extracted.data.lineItems.map((item, i) => (
                                    ...
                                    >
                                        <span>
                                            ...
                                            {item.fiber && (
                                                <span className="text-muted-foreground ml-2">({item.fiber})</span>
                                            )}
                                        </span>
